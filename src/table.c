...
double minimum, maximum;
  ...
  if ((table.has_row_names = has_row_names)){
    alloc_2D((void***)&table.row_names, nrow_buf, STRLEN, sizeof(char));
  } else {
  ...
  if ((table.has_col_names = has_col_names)){
    alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, sizeof(char));
  } else {
  ...
  // read line by line
  while (fgets(buffer, LONGSTRLEN, fp) != NULL){
    ...
    while (ptr != NULL){
      ...
      if (row == 0 && has_col_names && table.ncol == 0){
        ...
        copy_string(table.col_names[col], STRLEN, ptr);
        ptr = strtok(NULL, separator);
        col++;

        // if too many cols, add twice of previous cols to buffer
        if (col >= ncol_buf){
          re_alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, ncol_buf*2, STRLEN, sizeof(char));
        ...
        // if there is no other item after, exit with error
        if (has_row_names && col == 0){
          copy_string(table.row_names[row], STRLEN, ptr);
          if ((ptr = strtok(NULL, separator)) == NULL){
        ...
        // if too many cols, add twice of previous cols to buffer
        if (col >= ncol_buf){
          if (has_col_names) re_alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, ncol_buf*2, STRLEN, sizeof(char));
          re_alloc_2D((void***)&table.data, nrow_buf, ncol_buf, nrow_buf, ncol_buf*2, sizeof(double));
          ncol_buf *= 2;
        }
    ...
    if (row >= (int)nrow_buf){
      ...
      if (has_row_names) re_alloc_2D((void***)&table.row_names, nrow_buf, STRLEN, nrow_buf*2, STRLEN, sizeof(char));
      nrow_buf *= 2;
    }
  ...
  if (table.nrow != nrow_buf || table.ncol != ncol_buf){
    ...
    if (has_row_names) re_alloc_2D((void***)&table.row_names, nrow_buf, STRLEN, table.nrow, STRLEN, sizeof(char));
    if (has_col_names) re_alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, table.ncol, STRLEN, sizeof(char));
  }
...
/** This function reads a string table.
--- fname:         text file
--- has_row_names: Has the table row    names? true/false
--- has_col_names: Has the table column names? true/false
+++ Return:        table
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++**/
table_t read_str_table(char *fname, bool has_row_names, bool has_col_names){
FILE *fp;
table_t table;
char  buffer[LONGSTRLEN] = "\0";
char *ptr = NULL;
...
int ncol_buf = 1;
  ...
  // allocate table data
  alloc_3D((void****)&table.str_data, nrow_buf, ncol_buf, STRLEN, sizeof(char));
  ...
  while (fgets(buffer, LONGSTRLEN, fp) != NULL){
    ...
    while (ptr != NULL){
      ...
      if (row == 0 && has_col_names && table.ncol == 0){
        ...
        if (col >= ncol_buf){
          re_alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, ncol_buf*2, STRLEN, sizeof(char));
          re_alloc_3D((void****)&table.str_data, nrow_buf, ncol_buf, STRLEN, nrow_buf, ncol_buf*2, STRLEN, sizeof(char));
          ncol_buf *= 2;
        }
        ...
        copy_string(table.str_data[row][col], STRLEN, ptr);
        ptr = strtok(NULL, separator);
        col++;
    ...
    // if too many rows, add twice of previous rows to buffer
    if (row >= (int)nrow_buf){
      re_alloc_3D((void****)&table.str_data, nrow_buf, ncol_buf, STRLEN, nrow_buf*2, ncol_buf, STRLEN, sizeof(char));
      if (has_row_names) re_alloc_2D((void***)&table.row_names, nrow_buf, STRLEN, nrow_buf*2, STRLEN, sizeof(char));
      nrow_buf *= 2;
    }
  ...
  // re-shape buffer to actual dimensions, 1st rows, then cols
  if (table.nrow != nrow_buf || table.ncol != ncol_buf){
    re_alloc_3D((void****)&table.str_data, nrow_buf,   ncol_buf, STRLEN, table.nrow, ncol_buf, STRLEN, sizeof(char));
    re_alloc_3D((void****)&table.str_data, table.nrow, table.ncol, STRLEN, table.nrow, table.ncol, STRLEN, sizeof(char));
    if (has_row_names) re_alloc_2D((void***)&table.row_names, nrow_buf, STRLEN, table.nrow, STRLEN, sizeof(char));
    if (has_col_names) re_alloc_2D((void***)&table.col_names, ncol_buf, STRLEN, table.ncol, STRLEN, sizeof(char));
  }
...
table_t table;
  ...
  // allocate row names
  if ((table.has_row_names = has_row_names)){
    alloc_2D((void***)&table.row_names, table.nrow, STRLEN, sizeof(char));
  } else {
    table.row_names = NULL;
  }
  ...
  // allocate column names
  if ((table.has_col_names = has_col_names)){
    alloc_2D((void***)&table.col_names, table.ncol, STRLEN, sizeof(char));
  } else {
    table.col_names = NULL;
  }
...
table_t allocate_str_table(int nrow, int ncol, bool has_row_names, bool has_col_names){
table_t table;
  ...
  // allocate table data
  alloc_3D((void****)&table.str_data, table.nrow, table.ncol, STRLEN, sizeof(char));
...
void print_str_table(table_t *table, bool truncate, bool skip_rows){
int row, col;
int nrow_print = 6, ncol_print = 6;
int width, *max_width = NULL;
  ...
  for (col=0; col<table->ncol; col++){
    ...
    for (row=0; row<table->nrow; row++){
        width = strlen(table->str_data[row][col]); 
        if (width > max_width[col+1]) max_width[col+1] = width;
    }
  ...
  for (row=0; row<nrow_print; row++){
    ...
    if (table->has_row_names) printf("%*s ", max_width[0], table->row_names[row]);
    for (col=0; col<ncol_print; col++) printf("%*s ", max_width[col+1], table->str_data[row][col]);
    if (truncate && col < table->ncol) printf("...");
    printf("\n");
...
void write_str_table(table_t *table, char *fname, const char *separator, bool skip_rows){
int row, col;
FILE *fp = NULL;
  ...
  for (row=0; row<table->nrow; row++){
    ...
    if (table->has_row_names) fprintf(fp, "%s%s", table->row_names[row], separator);
    for (col=0; col<(table->ncol-1); col++) fprintf(fp, "%s%s", table->str_data[row][col], separator);
    fprintf(fp, "%s\n", table->str_data[row][col]);
...
void free_table(table_t *table){
  ...
  if (table->str_data != NULL){
    free_3D((void***)table->str_data, table->nrow, table->ncol);
    table->str_data = NULL;
  }
